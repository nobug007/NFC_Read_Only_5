...
void print_each_data_1(){
   ...
   while(i<data_count-1) {    // 데이터가 FE까지 포함 하므로, 하나를 뺐음.. -- 사실 밑에 FE와 같으면 break 하는 루틴은 없어도 됨.
          if ( (Read_Data[i-1] == 0x6E && Read_Data[i-2] == 0x65 ) &&  Read_Data[i-3] == 0x02 ) {     // 하나의 데이터 사이즈를 데이터로 가지고 있으나, 이를 계산 하기 힘들어서(조금씩 달라지고 큰 수의 경우 패턴이 달라져서. ㅜ.ㅜ ) "en" 이라는 것 다음에 데이터가 쓰여지는 것을 보고, 이를 체크 함. 
